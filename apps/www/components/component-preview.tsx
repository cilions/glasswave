...
export function ComponentPreview({ children, label }: ComponentPreviewProps) {
  return (
    <div className="my-6 rounded-xl overflow-hidden border border-white/10 dark:border-white/10">
      {label && (
        <div className="px-4 py-2 text-xs font-medium text-fd-muted-foreground border-b border-white/10 dark:border-white/10 bg-fd-card">
          {label}
        </div>
      )}
      <div className="relative flex flex-wrap min-h-[160px] items-center justify-center p-10 bg-fd-background bg-[url('https://mdx.fumadocs.com/grid.svg')] dark:bg-[url('https://mdx.fumadocs.com/grid-dark.svg')]">
        {/* overlay to ensure contrast and readability against pure grid */}
